...
#include "action_softbus/bebop_action.h"

// Register the nodelet
PLUGINLIB_EXPORT_CLASS(action_softbus::BebopAction, action_softbus::ActionCore)


namespace action_softbus{

    BebopAction::BebopAction() {}

    void BebopAction::initialize(std::string name, tf::TransformListener* tf, costmap_2d::Costmap2DROS* costmap_ros)
    {
        ros::NodeHandle nh;
        ...
        leader_vel_sub_ = nh.subscribe<geometry_msgs::Twist>("/leader_cmd_vel", 1000, &BebopAction::leaderVelCB, this);
    }

    void BebopAction::leaderVelCB(const geometry_msgs::Twist::ConstPtr& swarm_vel)
    {
        leader_vel_queue_.push(*swarm_vel);
    }


    bool BebopAction::setPath(const std::vector<decide_softbus_msgs::NavigationPoint>& path)
    {
        path_ = path;
        return true;
    }

    bool BebopAction::computeVelCmd(geometry_msgs::Twist& vel_cmd)
    {
         ...
         */
        static int count=0;
        double mypx = bebop_flock::my_position.first;
        double mypy = bebop_flock::my_position.second;
        int index =0;
    ...
    bool BebopAction::isGoalReached()
    {
        return false;
    }
}
