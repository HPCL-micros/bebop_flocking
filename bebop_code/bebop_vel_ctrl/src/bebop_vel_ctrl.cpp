...
BebopVelCtrl::BebopVelCtrl(ros::NodeHandle &nh)
  : nh_(nh),
    nh_priv_("~"),
    nh_pid_vx_(nh_priv_, "pid_forward"),
    nh_pid_vy_(nh_priv_, "pid_lateral"),
    ...
    sub_bebop_alt_(nh_, "states/ardrone3/PilotingState/AltitudeChanged", 1),
    sub_bebop_att_(nh_, "states/ardrone3/PilotingState/AttitudeChanged", 1),
    sub_bebop_speed_(nh_, "states/ardrone3/PilotingState/SpeedChanged", 1),
                                     ...
                                     const bebop_msgs::Ardrone3PilotingStateSpeedChangedConstPtr& speed_ptr)
{
  ...
  if (!beb_param_recv)
  {
    ...
    if (!util::GetParam(nh_, "bebop_driver/PilotingSettingsMaxTiltCurrent", beb_maxtilt_rad_))
    {
      return;
    }
    beb_maxtilt_rad_ = angles::from_degrees(beb_maxtilt_rad_);
    if (!util::GetParam(nh_, "bebop_driver/SpeedSettingsMaxVerticalSpeedCurrent", beb_max_speed_vert_m_))
    {
      return;
    }
    if (!util::GetParam(nh_, "bebop_driver/SpeedSettingsMaxRotationSpeedCurrent", beb_max_speed_rot_rad_))
    {
      return;
    }
